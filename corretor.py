...
def alimentaDetalhesRelatorio(lista, data, descricao, valor, correcao, corrigido, juros, total):
    lista.append({"data": data, "descricao": descricao, "valor": valor, "correcao": correcao, "corrigido": corrigido,
                  "juros": juros, "total": total})
...
def telaAlcadas(parametrosGerais=False):
    ...
    alcadas = [
        [sg.Text(text='Definição de Alçadas', text_color="Black", font=("Arial", 12, "bold"), expand_x=True,
                 justification='center')],
        [sg.Table(values=dados, headings=['Alçada', 'Perc. %'], auto_size_columns=True, display_row_numbers=False,
                  justification='center', key='-TABLE_ALCADAS-', selected_row_colors='red on yellow',
                  enable_events=True, expand_x=True, expand_y=True, enable_click_events=True)],
        [sg.Text(text='Alçada %: '), sg.Input(key='ed_valor_alcada', size=(15, 1), enable_events=True),
         sg.Button(' + Adicionar', key='btn_incluir_alcadas'), sg.Button(' - Remover', key='btn_remover_alcadas')],
...
def main():
    ...
    lista_informacoes.append(
        sg.Multiline(default_text=informacoes, expand_x=True, size=(None, 5), key="ed_informacoes", enable_events=True,
                     auto_refresh=True))
    frame_informacoes = sg.Frame('Informações da Ficha Gráfica Selecionada', [lista_informacoes], expand_x=True,
                                 visible=True, key='Info')
    ...
    lista_multa.append(sg.Combo(['Sobre o Valor Corrigido+Juros Principais', 'Sobre o Valor Corrigido',
                                 'Sobre o Valor Original+Juros Principais', 'Sobre o Valor Original'],
                                key="ed_multa_incidencia", enable_events=True))
    ...
    lista_multa_parametros.append(sg.Combo(['Sobre o Valor Corrigido+Juros Principais', 'Sobre o Valor Corrigido',
                                            'Sobre o Valor Original+Juros Principais', 'Sobre o Valor Original'],
                                           key="ed_multa_incidencia_param", enable_events=True))
    ...
    frame_honorarios_parametros = sg.Frame('Honorários', [lista_honorarios_parametros], expand_x=True,
                                           key="frame_honorarios_param")

    lista_outros_parametros = []
    lista_outros_parametros.append(sg.Text(text="Valor R$:", font=("Arial", 10, "bold")))
    lista_outros_parametros.append(
        sg.Input(key="ed_outros_valor_param", size=(20, 1), default_text='0,00', enable_events=True))
    frame_outros_parametros = sg.Frame('Outros Valores', [lista_outros_parametros], expand_x=True)

    ## Itens da aba Principal
    principal = [
        [sg.Text(text='Corretor Monetário', text_color="Black", font=("Arial", 22, "bold"), expand_x=True,
                 justification='center')],
        [frame_arquivo],
        [frame_informacoes],
        [frame_indices],
        ...
        [sg.Button('Calcular'), sg.Button('Fechar'), sg.Button('Testar'), sg.Text(
            'Host: ' + maquina + '   |   IP: ' + ip + '   |   ' + internet + '   |   Versão: ' + versao_exe + ' - ' + versao_descricao,
            expand_x=True, justification='right', font=("Verdana", 8))]
    ]
    ...
    importadas = [
        [sg.Text(text='Fichas Importadas', text_color="black", font=("Arial", 12), expand_x=True,
                 justification='center')],
        [frame_filtros],
        [sg.Table(values=fichas_importadas,
                  headings=['ID', 'Título', 'Versão', 'Associado', 'N° Parcelas', 'Valor Financiado', 'Taxa Juros'],
                  auto_size_columns=True, display_row_numbers=False, justification='center', key='-TABLE-',
                  selected_row_colors='red on yellow', enable_events=True, expand_x=True, expand_y=True,
    ...
    parametros = [
        ...
        [sg.Text(text='Parâmetros Padrão do Sistema', text_color="black", font=("Arial", 9), expand_x=True,
                 justification='left')],
        [sg.Text(
            text='Atenção: Opções definidas aqui serão carregadas como padrão para todos ao inicializar uma nova importação.',
            text_color="red", font=("Verdana", 7), expand_x=True, justification='left')],
    ...
    tabgrp = [
        [sg.TabGroup
            (
            [
                [sg.Tab('Principal', principal, title_color='Red', border_width=10, key='aba_principal'),
                 sg.Tab('Importadas', importadas, key='aba_importadas'),
                 sg.Tab('Parâmetros', parametros, key='aba_parametros')]
    ...
    if (busca_licena == 'THMPV-77D6F-94376-8HGKG-VRDRQ'):
        ...
        while True:
            ...
            if eventos == 'btn_salvar_parametros_gerais':
                ...
                try:
                    if f.salvarParametrosGerais(valores['ed_igpm_param'], valores['ed_ipca_param'],
                                                valores['ed_cdi_param'], valores['ed_inpc_param'],
                                                valores['ed_tr_param'], vmulta_perc_param, vmulta_valor_param,
                                                vincidencia_param, vhonorarios_perc_param, vhonorarios_valor_param,
                                                voutros_valor_param):
                        carregaParametros(tela)
            ...
            if eventos == 'Calcular':
                ...
                if valores['arquivo'] != '':
                    ...
                    try:
                        ...
                        f.gravalog(
                            'Falha ao tentar remover o arquivo txt temporário(Conversão PDF para Txt). ' + str(erro))
